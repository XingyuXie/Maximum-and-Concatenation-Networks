...
class CifarResNeXt_MCN(nn.Module):
    ...
    def __init__(self, cardinality, depth, nlabels, base_width, widen_factor=4):
        ...
        self.MCN_block1 = models.MCN_block(1024, 1024, 512, 1536, x0_stride=2, bn_pool_flag = True)
        self.MCN_block2 = models.MCN_block(1024, 2048, 512, 1536, x0_stride=2, bn_pool_flag = True)
    ...
    def forward(self, x):
        ...
        x = self.stage_1.forward(x) # 256 * 32 * 32
        x = self.stage_2.forward(x) # 512 * 16 * 16
        x = self.stage_3.forward(x)# 1024 * 8 * 8
        ...
        x_kk= self.MCN_block1(x_0, x_k) # 2048 * 4 * 4
        x_final = self.MCN_block2(x_0, x_kk) # 2048 * 4 * 4
                
        x_final = self.avg(x_final) # 2048 * 2 * 2
        ...
        #x_7 = self.pool(x_6)
        x_final = avg_x_final.view(avg_x_final.size(0), -1)
